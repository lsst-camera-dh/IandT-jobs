...
class DrainSweepAcquisition(EOAcquisition):
    """
    ...
    def __init__(self, seqfile, acq_config_file, metadata, subsystems,
        ...
        super(DrainSweepAcquisition, self).__init__(seqfile, acq_config_file,
                                                           "DRAINSWEEP", metadata,
    ...
    def run(self):
        ...
        drainsweep 60 10 25.0 28.0 0.5 12.0 14.0 0.5 
        """
        ...
        for tokens in self.instructions:
            ...
            test_type = "DRAINSWEEP" # Change to desired test name
            ...
            for voltage_pair in voltage_pairs:
                ...
                for iframe in range(nframes):
                    ...
                    fits_files = self.take_image(seqno, exptime, openShutter, 
                                                 ...
                                                 test_type=test_type,
                                                 file_template=filte_template)
                    seqno += 1

if __name__ == '__main__':

    metadata = AcqMetadata(cwd=tsCWD, raft_id=UNITID, run_number=RUNNUM)
    acq = DrainSweepAcquisition(sequence_file, rtmacqcfgfile, metadata, subsystems,
                                       ccd_names)
    acq.run()

...
class SerialSweepAcquisition(EOAcquisition):
    """
    ...
    def __init__(self, seqfile, acq_config_file, metadata, subsystems,
        ...
        super(SerialSweepAcquisition, self).__init__(seqfile, acq_config_file,
                                                     "SERIALSWEEP", metadata,
    ...
    def run(self):
        ...
        wl = float(self.eo_config.get("SERIALSWEEP_WL", 500))
        ...
        for tokens in self.instructions:
            ...
            test_type = "SERIALSWEEP" % wl # Change to desired test name
            ...
            serlo_values = list(np.arange(min_serlo, max_serlo+0.1, step_serlo)) # Potential danger with inexact floats
            serhi_values = list(np.arange(min_serhi, max_serhi+0.1, step_serhi)) # Potential danger with inexact floats
...
if __name__ == '__main__':
    ...
    acq = SerialSweepAcquisition(sequence_file, rtmacqcfgfile, metadata, subsystems,
                                       ccd_names)
    acq.run()
